  ...
  return function(content, done){
    each(
      ...
      function(err){
        ...
        // TODO jjalan: Stream created using Blip causes issues for some reason with archiver
        // For now, create a simply stream using inbuilt nodejs support
        // serialize the xml data into a stream and return it
        var Readable = require('stream').Readable

        var s = new Readable
        s.push(xmlString)    // the string you want
        s.push(null)      // indicates end-of-file basically - the end of the stream
        done(null, s);
        //done(null, new Blip(xmlString));
      }
    );
  };
};
